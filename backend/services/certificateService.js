...
const generateCertificate = async ({ userName, eventTitle, eventDate, eventLocation, certificateId }) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Date and Location — plain text, no emojis
      const formattedDate = new Date(eventDate).toLocaleDateString('en-US', {
        year: 'numeric', month: 'long', day: 'numeric',
      });

      doc.font('Helvetica-Bold').fontSize(9).fillColor('#475569')
         .text('DATE', W*0.28, 286, { width: 60, align: 'center' });
      doc.font('Helvetica').fontSize(10).fillColor('#94a3b8')
         .text(formattedDate, W*0.22, 298, { width: 180, align: 'center' });

      // Vertical separator between date and location
      doc.moveTo(W*0.5, 283).lineTo(W*0.5, 312).lineWidth(0.5).stroke('#334155');

      doc.font('Helvetica-Bold').fontSize(9).fillColor('#475569')
         .text('LOCATION', W*0.5, 286, { width: 120, align: 'center' });
      doc.font('Helvetica').fontSize(10).fillColor('#94a3b8')
         .text(eventLocation, W*0.5, 298, { width: 180, align: 'center' });

      // Horizontal rule
      doc.moveTo(60, 323).lineTo(W-60, 323).lineWidth(0.3).stroke('#1e3a4a');

      // Signature (left)
      doc.moveTo(110, 358).lineTo(270, 358).lineWidth(0.7).stroke('#334155');
      doc.font('Helvetica-Bold').fontSize(9).fillColor('#94a3b8')
         .text('Authorized Signatory', 110, 364, { width: 160, align: 'center' });
      doc.font('Helvetica').fontSize(8).fillColor('#475569')
         .text('NGO Events Platform', 110, 377, { width: 160, align: 'center' });

      // Center seal
      doc.circle(W/2, 352, 32).lineWidth(1.5).stroke('#22d3ee');
      doc.circle(W/2, 352, 26).lineWidth(0.5).stroke('#1e4a5a');
      doc.font('Helvetica-Bold').fontSize(7).fillColor('#22d3ee')
         .text('VERIFIED',    W/2 - 16, 344);
      doc.font('Helvetica-Bold').fontSize(7).fillColor('#94a3b8')
         .text('CERTIFICATE', W/2 - 20, 354);
      doc.font('Helvetica').fontSize(6).fillColor('#475569')
         .text('NGO EVENTS',  W/2 - 14, 364);
